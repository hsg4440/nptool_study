...
void Analysis_nptool(){
    ...
    auto lat = new TLatex();
    TH2D *dEdX_hist = new TH2D("Energy_Loss","dE/dx;Energy/u [MeV]; dE/dx [MeV/mm]",400,0,2000,400,0,0.0003);
    ...
    for(int event = 0 ; event < eventnum; event++){
        ...
        for(int i = 0 ; i < interaction_Size-1 ; i++){
            // cout <<data->GetDataMaterial(i) << endl;
    ...
    dEdX_hist->GetZaxis()->SetRangeUser(0,1000);
    dEdX_hist->Draw("colZ");
    // loss_energy->Draw();
